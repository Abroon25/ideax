...
function IdeaCard({ idea, onLike, onBookmark, onImageClick, onRepost }) {
  ...
  return (
    <div className="border-b border-dark-700 p-4 hover:bg-dark-900/30 transition-colors cursor-pointer" onClick={(e) => { if (!e.target.closest('button') && !e.target.closest('a')) navigate('/idea/' + displayIdea.id); }}>
      ...
      {/* NEW: Thread Context Header */}
      {displayIdea.parentIdeaId && (
        <div className="flex items-center gap-2 text-dark-400 text-[13px] font-bold mb-1 ml-9">
      ...
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-center">
          ...
          {displayIdea._count?.replies > 0 && <div className="w-[2px] h-full bg-dark-700 mt-2"></div>}
        </div>
...
export default function Home() {
  ...
  const handlePost = async () => {
    ...
    if (!replyingTo) formData.append('genreId', selectedGenre);
    if (replyingTo) formData.append('parentIdeaId', replyingTo.id);
    ...
    try {
      ...
      toast.success(replyingTo ? 'Reply posted!' : 'Idea sent!');
      
      // Add to feed instantly
      setIdeas(prev => [res.data.idea, ...prev]);
      
      // Update parent reply count in UI
      if (replyingTo) {
        ...
        <div className="fixed inset-0 z-[150] flex items-start sm:items-center justify-center bg-dark-950/80 sm:bg-dark-950/60 backdrop-blur-sm pt-0 sm:pt-10">
          <div className="bg-dark-950 sm:bg-dark-900 w-full max-w-[600px] h-full sm:h-auto sm:rounded-2xl shadow-2xl flex flex-col relative sm:border border-dark-700">
            <div className="flex items-center justify-between px-4 py-3 border-b border-dark-700 sm:border-transparent">
              <button onClick={() => setReplyingTo(null)} className="p-2 hover:bg-dark-800 rounded-full transition-colors text-white"><HiOutlineX className="w-5 h-5" /></button>
              <div className="sm:hidden"><button onClick={handlePost} disabled={!content.trim() || posting} className="bg-primary-500 text-white font-bold py-1.5 px-5 rounded-full disabled:opacity-50">Reply</button></div>
            </div>

            <div className="p-4 flex gap-3 overflow-y-auto">
              <div className="flex flex-col items-center">
                <Avatar src={replyingTo.author.avatar} name={replyingTo.author.displayName} />
                <div className="w-[2px] h-full bg-dark-700 my-2"></div>
                <Avatar src={user?.avatar} name={user?.displayName} />
              </div>
              <div className="flex-1 min-w-0 pb-10">
                <div className="mb-4">
                  <div className="flex items-center gap-2"><span className="font-bold text-white">{replyingTo.author.displayName}</span><span className="text-dark-400">@{replyingTo.author.username}</span></div>
                  <p className="text-[15px] text-white mt-1 line-clamp-3"><RichText text={replyingTo.content} /></p>
                  <p className="text-[15px] text-dark-400 mt-2">Replying to <span className="text-primary-500">@{replyingTo.author.username}</span></p>
                </div>
                <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Post your reply" className="w-full bg-transparent text-xl placeholder-dark-500 resize-none focus:outline-none min-h-[120px] text-white" maxLength={limits.maxChars} autoFocus />
              </div>
            </div>
            ...
            <div className="border-t border-dark-700 p-3 flex items-center justify-between sticky bottom-0 bg-dark-950 sm:bg-dark-900 sm:rounded-b-2xl">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handlePost} disabled={!content.trim() || posting} className="hidden sm:block bg-primary-500 hover:bg-primary-600 text-white font-bold py-1.5 px-5 rounded-full disabled:opacity-50">Reply</button>
              </div>
            </div>
          </div>
        </div>
