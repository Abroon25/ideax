...
import { HiMagnifyingGlass, HiOutlineBookmark as HiOutlineBM } from 'react-icons/hi2';
...
export default function Home() {
    ...
    api.get('/ideas/feed', { params: { page: p, limit: 20 } })
      .then(function(res) {
        if (p === 1) {
          ...
          setIdeas(function(prev) { return prev.concat(res.data.ideas); });
        }
          ...
          return prev.map(function(i) {
            if (i.id === id) {
              return Object.assign({}, i, {
                ...
                _count: Object.assign({}, i._count, {
                  likes: i._count.likes + (res.data.liked ? 1 : -1)
                })
              });
            }
            return i;
  ...
  function handleBookmark(id) {
    api.post('/ideas/' + id + '/bookmark')
      .then(function(res) {
        setIdeas(function(prev) {
          return prev.map(function(i) {
            if (i.id === id) {
              return Object.assign({}, i, { isBookmarked: res.data.bookmarked });
            }
            return i;
          });
        });
  ...
  function finishTour() {
    api.post('/users/complete-tour')
      .then(function() { updateUser({ tourCompleted: true }); })
      .catch(function() {});
    setShowTour(false);
  }
  ...
  var tourSteps = [
    ...
    { icon: '⭐', title: 'Tier System', desc: 'FREE: 500 chars. BASIC: 15K chars + files + money/profit. PREMIUM: 50K chars + all options + legal support.' },
    { icon: '🌟', title: 'Ready!', desc: 'Start exploring ideas or post your first brilliant idea!' }
  ];

  var navLinks = [
    { to: '/', icon: HiOutlineHome, label: 'Home' },
    { to: '/search', icon: HiMagnifyingGlass, label: 'Search' },
  ...
  return (
    <div className="flex justify-center min-h-screen bg-dark-950">
          ...
          <div>
            ...
            <nav className="space-y-1">
              {navLinks.map(function(item) {
                var Icon = item.icon;
                return (
                  <NavLink key={item.to} to={item.to} className={function(props) { return 'sidebar-link ' + (props.isActive ? 'active font-bold' : 'text-dark-300'); }}>
                    <Icon className="w-7 h-7" /><span>{item.label}</span>
                  </NavLink>
                );
              })}
            </nav>
            <button onClick={function() { setShowComposer(true); }} className="btn-primary w-full mt-4 py-3 text-lg flex items-center justify-center gap-2">
              <HiOutlineLightBulb className="w-5 h-5" /> Post Idea
            </button>
          </div>
          <button onClick={function() { navigate('/profile/' + (user ? user.username : '')); }} className="flex items-center gap-3 p-3 rounded-full hover:bg-dark-800 transition-colors mb-2">
      ...
      <div className="hidden xl:block w-[350px]">
        <div className="fixed w-[350px] h-screen py-4 px-6 overflow-y-auto">
          ...
          <div className="card p-4">
            ...
            {genres.slice(0, 6).map(function(g) {
              return (
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-dark-900 border-t border-dark-700 flex justify-around py-2 lg:hidden z-40">
        <NavLink to="/" className={function(p) { return 'p-3 ' + (p.isActive ? 'text-primary-500' : 'text-dark-400'); }}>
